...
void IndexBipartite::RefreshProjectionSupportIndex(const Parameters &parameters) {
    const uint32_t support_topk = parameters.Get<uint32_t>("M_sq");
    support_queries_by_base_.clear();
    support_queries_by_base_.resize(u32_nd_);

    const uint32_t num_queries = std::min<uint32_t>(u32_nd_sq_, static_cast<uint32_t>(learn_base_knn_.size()));
    for (uint32_t sq = 0; sq < num_queries; ++sq) {
        const auto &nn_base = learn_base_knn_[sq];
        const size_t topk = std::min<size_t>(support_topk, nn_base.size());

        tsl::robin_set<uint32_t> seen_in_query;
        seen_in_query.reserve(topk);
        for (size_t i = 0; i < topk; ++i) {
            uint32_t base_id = nn_base[i];
            if (base_id >= u32_nd_ || !seen_in_query.insert(base_id).second) {
                continue;
            }
            support_queries_by_base_[base_id].push_back(sq);
        }
    }
}
...
void IndexBipartite::BuildRoarGraph(size_t n_sq, const float *sq_data, size_t n_bp, const float *bp_data,
                                    const Parameters &parameters, const char *projection_save_path_pre_ce) {
    std::cout << "start build bipartite index" << std::endl;
    auto s = std::chrono::high_resolution_clock::now();
    uint32_t M_sq = parameters.Get<uint32_t>("M_sq");
    // uint32_t M_bp = parameters.Get<uint32_t>("M_bp");
    ...
    assert(projection_ep_ < nd_);
    std::cout << "begin link projection" << std::endl;
    LinkProjection(parameters, projection_save_path_pre_ce);
    std::cout << std::endl;
    ...
    auto diff = e - s;
    std::cout << "Build projection graph time: " << diff.count() / (1000 * 1000 * 1000) << std::endl;
    // ======= 这里是 projected graph 已完成、CE 尚未开始的最佳保存点 =======
    if (projection_save_path_pre_ce != nullptr && projection_save_path_pre_ce[0] != '\0') {
        SaveProjectionGraph(projection_save_path_pre_ce);
        std::cout << "[RoarGraph] Saved projected graph (pre-CE) to "
                  << projection_save_path_pre_ce << std::endl;

        // 如果你想“只构到这里就停止”，可以直接 return;
        // return;
    }
...
void IndexBipartite::LinkProjection(const Parameters &parameters, const char *projection_save_path_pre_ce) {
    (void)projection_save_path_pre_ce;
    uint32_t M_pjbp = parameters.Get<uint32_t>("M_pjbp");
    uint32_t L_pjpq = parameters.Get<uint32_t>("L_pjpq");
    uint32_t Nq = parameters.Get<uint32_t>("M_sq");

    omp_set_num_threads(parameters.Get<uint32_t>("num_threads"));
    RefreshProjectionSupportIndex(parameters);
    std::vector<uint32_t> vis_order;
    std::vector<uint32_t> vis_order_sq;
    for (uint32_t i = 0; i < u32_nd_; ++i) {
        vis_order.push_back(i);
...
#pragma omp parallel for schedule(static, 100)
    for (uint32_t it_sq = 0; it_sq < u32_nd_sq_; ++it_sq) {
        ...
        const auto &nn_base = learn_base_knn_[sq];
        const size_t topk = std::min<size_t>(Nq, nn_base.size());
        if (topk == 0) {
            continue;
        }
        std::vector<uint32_t> valid_nn_base;
        valid_nn_base.reserve(topk);
        tsl::robin_set<uint32_t> seen_base;
        seen_base.reserve(topk);
        for (size_t i = 0; i < topk; ++i) {
            uint32_t base_id = nn_base[i];
            if (base_id >= u32_nd_ || !seen_base.insert(base_id).second) {
                continue;
            }
            valid_nn_base.push_back(base_id);
        }
        if (valid_nn_base.empty()) {
            continue;
        }

        uint32_t cur_tgt = valid_nn_base[0];
        std::vector<Neighbor> full_retset;
        full_retset.reserve(valid_nn_base.size() > 0 ? valid_nn_base.size() - 1 : 0);
        for (size_t i = 0; i < valid_nn_base.size(); ++i) {
            uint32_t candidate = valid_nn_base[i];
            if (candidate == cur_tgt) {
                continue;
            }
            float distance = distance_->compare(data_bp_ + dimension_ * (uint64_t)candidate,
                                                data_bp_ + dimension_ * (uint64_t)cur_tgt,
                                                (unsigned)dimension_);
            full_retset.push_back(Neighbor(candidate, distance, false));
        }
        std::vector<uint32_t> pruned_list;
        pruned_list.reserve(M_pjbp * PROJECTION_SLACK);
        PruneBiSearchBaseGetBase(full_retset, data_bp_ + dimension_ * cur_tgt, cur_tgt, parameters, pruned_list);
...
#pragma omp parallel for schedule(static, 2048)
    for (uint32_t i = 0; i < vis_order.size(); ++i) {
        ...
        if (projection_graph_[node].size() > M_pjbp) {
            // After reverse edges are added, prune again with the same
            // support-based top-M rule to bring the degree back to M_pjbp.
            std::vector<Neighbor> full_retset;
...
void IndexBipartite::SupplyAddReverse(uint32_t src_node, const Parameters &parameters) {
    ...
    {
        LockGuard guard(locks_[src_node]);
        nbrs = supply_nbrs_[src_node];
    }
...
void IndexBipartite::ProjectionAddReverse(uint32_t src_node, const Parameters &parameters) {
    ...
    {
        LockGuard guard(locks_[src_node]);
        nbrs = projection_graph_[src_node];
    }
    float avg_copy = 0;
...
//     pruned_list = result;
// }
void IndexBipartite::PruneProjectionReverseCandidates(uint32_t src_node,
                                                      const Parameters &parameters,
                                                      std::vector<uint32_t> &pruned_list) {
    const uint32_t M_pjbp = parameters.Get<uint32_t>("M_pjbp");
    const uint32_t support_topk = parameters.Get<uint32_t>("M_sq");
    ...
    candidates.reserve(pruned_list.size());

    tsl::robin_map<uint32_t, size_t> candidate_pos;
    candidate_pos.reserve(pruned_list.size());

    // 1) 去重，并过滤 self-loop
    for (uint32_t candidate : pruned_list) {
        if (candidate == src_node) continue;
        if (candidate_pos.find(candidate) != candidate_pos.end()) continue;

        float distance = distance_->compare(
            data_bp_ + dimension_ * (size_t)src_node,
            data_bp_ + dimension_ * (size_t)candidate,
            (unsigned)dimension_);

        candidate_pos[candidate] = candidates.size();
        candidates.push_back({candidate, 0, distance});
    }
    ...
    // 2) support(src_node, candidate)
    // support = 有多少历史 query 同时把 src_node 和 candidate 放进 top-M_sq
    if (src_node < support_queries_by_base_.size()) {
        for (uint32_t sq : support_queries_by_base_[src_node]) {
            if (sq >= learn_base_knn_.size()) continue;

            const auto &nn_base = learn_base_knn_[sq];
            const size_t topk = std::min<size_t>(support_topk, nn_base.size());

            for (size_t i = 0; i < topk; ++i) {
                uint32_t candidate = nn_base[i];
                if (candidate == src_node) continue;

                auto it = candidate_pos.find(candidate);
                if (it != candidate_pos.end()) {
                    ++candidates[it->second].support;
                }
            }
        }
    }

    // 3) 按 support 降序；support 相同则按距离升序
    std::stable_sort(
    ...
    // 4) 直接取前 M_pjbp
    std::vector<uint32_t> result;
    result.reserve(std::min<size_t>(M_pjbp, candidates.size()));
    for (size_t i = 0; i < candidates.size() && result.size() < M_pjbp; ++i) {
        result.push_back(candidates[i].id);
    }

    pruned_list.swap(result);
}
void IndexBipartite::PruneBiSearchBaseGetBase(std::vector<Neighbor> &search_pool, const float *query, uint32_t tgt_base,
    ...
    std::vector<SupportCandidate> candidates;
    candidates.reserve(search_pool.size());

    tsl::robin_map<uint32_t, size_t> candidate_pos;
    candidate_pos.reserve(search_pool.size());

    // Keep the original candidate order from the caller and only change the
    // ranking rule: sort by support(x, c) in descending order.
    for (const auto &neighbor : search_pool) {
        const uint32_t candidate = neighbor.id;
        if (candidate == tgt_base) {
            continue;
        }
        if (candidate_pos.find(candidate) != candidate_pos.end()) {
            continue;
        }
        candidate_pos[candidate] = candidates.size();
    ...
    if (tgt_base < support_queries_by_base_.size()) {
        const uint32_t support_topk = parameters.Get<uint32_t>("M_sq");
        for (uint32_t sq : support_queries_by_base_[tgt_base]) {
            if (sq >= learn_base_knn_.size()) {
                continue;
            }
            const auto &nn_base = learn_base_knn_[sq];
            const size_t topk = std::min<size_t>(support_topk, nn_base.size());
    ...
    std::stable_sort(candidates.begin(), candidates.end(),
                     [](const SupportCandidate &lhs, const SupportCandidate &rhs) {
                         return lhs.support > rhs.support;
                     });

    pruned_list.clear();
    pruned_list.reserve(std::min<size_t>(M_pjbp, candidates.size()));
    for (size_t i = 0; i < candidates.size() && pruned_list.size() < M_pjbp; ++i) {
...
IndexBipartite::SearchStats IndexBipartite::SearchRoarGraph(
        const float *query,
        size_t k,
        size_t &qid,
        const Parameters &parameters,
    ...
    uint32_t L_pq = parameters.Get<uint32_t>("L_pq");
    bool enable_jump_search = parameters.Get<bool>("enable_jump_search");
    uint32_t max_jump_candidates = parameters.Get<uint32_t>("max_jump_candidates");  // 0 表示不限制
    ...
    while (search_queue.has_unexpanded_node()) {
        auto cur_check_node = search_queue.closest_unexpanded();
        uint32_t cur_id = cur_check_node.id;
        float cur_dist = cur_check_node.distance;
        uint32_t *cur_nbrs = projection_graph_[cur_id].data();

        ++stats.hops;

        for (size_t j = 0; j < projection_graph_[cur_id].size(); ++j) {
            uint32_t mid = *(cur_nbrs + j);

            if (visited_array[mid] == visited_array_tag) {
                ++stats.total_already_visited;
                continue;
            }
            ++stats.total_newly_visited;

            // mid发现即 visited
            visited_array[mid] = visited_array_tag;

            _mm_prefetch((char *)(data_bp_ + mid * dimension_), _MM_HINT_T0);

            float mid_dist = distance_->compare(data_bp_ + mid * dimension_, query, (unsigned)dimension_);
            ++stats.distance_cmps;

            // 不启用 jump：完全走原 greedy 风格
            if (!enable_jump_search) {
                search_queue.insert({mid, mid_dist, false});
                continue;
            }

            // 启用 jump：检查 mid 的邻居 nn
            std::vector<std::pair<uint32_t, float>> jump_candidates;
            jump_candidates.reserve(projection_graph_[mid].size());

            uint32_t nn_cmps_this_mid = 0;
            bool has_jump_neighbor = false;   // 是否存在满足 jump 条件的 nn
            uint32_t *mid_nbrs = projection_graph_[mid].data();

            for (size_t t = 0; t < projection_graph_[mid].size(); ++t) {
                uint32_t nn = *(mid_nbrs + t);

                _mm_prefetch((char *)(data_bp_ + nn * dimension_), _MM_HINT_T0);

                float nn_dist = distance_->compare(data_bp_ + nn * dimension_, query, (unsigned)dimension_);
                ++nn_cmps_this_mid;

                // jump 条件：nn 比 cur 和 mid 都更接近 query
                if (nn_dist < cur_dist && nn_dist < mid_dist) {
                    has_jump_neighbor = true;

                    // 只有未 visited 的 nn 才需要真正加入候选并后续入队
                    if (visited_array[nn] != visited_array_tag) {
                        jump_candidates.emplace_back(nn, nn_dist);
                    }
                }
            }

            if (has_jump_neighbor) {
                // 这个 mid 是 jump 点，即使满足条件的 nn 已经 visited，也不让 mid 入队
                ++stats.jump_mid_count;

                // 当前统计逻辑：只有 jump 成立时，nn 的比较才计入总 distance_cmps
                stats.distance_cmps += nn_cmps_this_mid;
                ...
                size_t push_cnt = jump_candidates.size();
                if (max_jump_candidates > 0) {
                    push_cnt = std::min(push_cnt, static_cast<size_t>(max_jump_candidates));
                }

                // 只有未 visited 的 nn 才实际入队；已 visited 的 nn 不重复入队
                for (size_t x = 0; x < push_cnt; ++x) {
                    uint32_t nn_id = jump_candidates[x].first;
                    float nn_dist = jump_candidates[x].second;

                    if (visited_array[nn_id] == visited_array_tag) {
                        continue;
                    }

                    visited_array[nn_id] = visited_array_tag;
                    search_queue.insert({nn_id, nn_dist, false});
                }

                // 不让 mid 入队
            } else {
                // 不存在任何满足 jump 条件的 nn，mid 按原 greedy 风格入队
                ++stats.non_jump_mid_count;
                search_queue.insert({mid, mid_dist, false});

                // 这个 mid 的 nn 虽然做了距离计算，但按统计要求，不计入 distance_cmps
            }
        }
    }
...
// IndexBipartite::SearchStats IndexBipartite::SearchRoarGraphNoOpt(
//         const float *query,
//         size_t k,
//         size_t &qid,
//         const Parameters &parameters,
//         unsigned *indices,
//         std::vector<float> &res_dists,
//         std::vector<TwoHopEvent> *event_log) {
//     (void)qid;

//     uint32_t L_pq = parameters.Get<uint32_t>("L_pq");
//     const size_t ef = (size_t)L_pq;

//     bool enable_jump_search = parameters.Get<bool>("enable_jump_search");
//     uint32_t max_jump_candidates = parameters.Get<uint32_t>("max_jump_candidates");

//     std::vector<uint32_t> init_ids;
//     init_ids.push_back(projection_ep_);

//     std::vector<uint8_t> visited((size_t)u32_nd_, 0);
...
//     // 统计满足 jump 条件的 mid 中，有多少是比 cur 更近/更远
//     size_t jump_mid_closer_count = 0;
//     size_t jump_mid_farther_count = 0;

//     for (auto &id : init_ids) {
//         float distance = distance_->compare(data_bp_ + id * dimension_, query, (unsigned)dimension_);
//         ++stats.distance_cmps;
//         ++stats.cur_distance_cmps;

//         cand.emplace(distance, id);
//         top.emplace(distance, id);
//         if (top.size() > ef) {
//             top.pop();
//         }
//         visited[id] = 1;
...
//         const float cur_dist = cur.first;
//         const uint32_t cur_id = cur.second;
//         uint32_t *cur_nbrs = projection_graph_[cur_id].data();
...
//             float mid_dist = distance_->compare(data_bp_ + mid * dimension_, query, (unsigned)dimension_);
//             ++stats.distance_cmps;
//             ++stats.mid_distance_cmps;

//             if (!enable_jump_search) {
//                 ++stats.non_jump_mid_count;

//                 if (top.size() < ef || mid_dist < lb) {
//                     cand.emplace(mid_dist, mid);
//                     top.emplace(mid_dist, mid);
//                     if (top.size() > ef) {
//                         top.pop();
//                     }
//                     lb = top.top().first;
...
//                 const bool is_jump = (nn_dist < cur_dist && nn_dist < mid_dist);

//                 // 所有被检查过的二跳结构
//                 if (event_log != nullptr) {
//                     event_log->push_back(TwoHopEvent{
//                         static_cast<uint32_t>(qid),
//                         cur_id,
//                         mid,
//                         nn,
//                         static_cast<uint8_t>(is_jump ? 1 : 0)
//                     });
//                 }

//                 if (is_jump) {
//                     has_jump_neighbor = true;
//                     jump_candidates.push_back({nn, nn_dist});
//                 }
//             }

//             if (has_jump_neighbor) {
//                 ++stats.jump_mid_count;
//                 stats.jump_nn_distance_cmps += nn_cmps_this_mid;

//                 // 只统计满足 jump 条件的 mid 中，mid 比 cur 更近/更远的情况
//                 if (mid_dist < cur_dist) {
//                     ++jump_mid_closer_count;
//                 } else if (mid_dist > cur_dist) {
//                     ++jump_mid_farther_count;
//                 }

//                 std::sort(jump_candidates.begin(), jump_candidates.end(),
//                           [](const JumpCandidate &a, const JumpCandidate &b) {
//                               return a.nn_dist < b.nn_dist;
//                           });
...
//                 for (size_t x = 0; x < push_cnt; ++x) {
//                     uint32_t nn_id = jump_candidates[x].nn;
//                     float nn_dist = jump_candidates[x].nn_dist;

//                     if (visited[nn_id]) {
//                         continue;
//                     }

//                     if (top.size() < ef || nn_dist < lb) {
//                         // nn 真入队时才标记
//                         visited[nn_id] = 1;
//                         cand.emplace(nn_dist, nn_id);
//                         top.emplace(nn_dist, nn_id);
//                         if (top.size() > ef) {
//                             top.pop();
//                         }
//                         lb = top.top().first;
...
//             } else {
//                 ++stats.non_jump_mid_count;
//                 stats.non_jump_nn_distance_cmps += nn_cmps_this_mid;
...
//     // 记录：满足 jump 条件的 mid 中，有多少比 cur 更近/更远
//     stats.mid_closer_count = static_cast<uint32_t>(jump_mid_closer_count);
//     stats.mid_farther_count = static_cast<uint32_t>(jump_mid_farther_count);

//     if (unlikely(top.size() < k)) {
//         std::stringstream ss;
//         ss << "not enough results: " << top.size() << ", expected: " << k;
//         throw std::runtime_error(ss.str());
//     }
...
//     for (size_t i = 0; i < k; ++i) {
//         indices[i] = buf[i].second;
//         res_dists[i] = buf[i].first;
//     }
...
IndexBipartite::SearchStats IndexBipartite::SearchRoarGraphNoOpt(
    ...
    const uint32_t L_pq             = parameters.Get<uint32_t>("L_pq");
    const size_t   ef               = static_cast<size_t>(L_pq);
    const bool     enable_jump      = parameters.Get<bool>("enable_jump_search");
    const uint32_t max_jump_cands   = parameters.Get<uint32_t>("max_jump_candidates");

    // ------------------------------------------------------------------
    // 三态 state + distance cache
    // ------------------------------------------------------------------
    enum State : uint8_t { UNSEEN = 0, SCORED = 1, ENQUEUED = 2 };

    const uint32_t N = u32_nd_;
    std::vector<State> state(N, UNSEEN);
    std::vector<float> dist_cache(N, 0.0f);   // 仅 SCORED/ENQUEUED 时有效
    ...
    std::priority_queue<DistId, std::vector<DistId>, MinCmp> cand;
    std::priority_queue<DistId>                               top;   // max-heap，用于维护 ef 个最近邻

    float lb = std::numeric_limits<float>::infinity();

    // 获取节点距离（带 cache，每个节点只真正计算一次）
    auto get_dist = [&](uint32_t x) -> float {
        if (state[x] == UNSEEN) {
            dist_cache[x] = distance_->compare(
                data_bp_ + x * dimension_, query, (unsigned)dimension_);
            ++stats.distance_cmps;
            ++stats.unique_nn_scored;
            state[x] = SCORED;
        } else {
            ++stats.duplicate_nn_skip;
        }
        return dist_cache[x];
    };

    // 尝试将节点写入 beam（cand + top），返回是否真正写入
    auto enqueue_node = [&](uint32_t id, float d) -> bool {
        ++stats.beam_candidate_count;
        if (top.size() < ef || d < lb) {
            if (state[id] != ENQUEUED) {
                state[id] = ENQUEUED;
                cand.emplace(d, id);
                top.emplace(d, id);
                if (top.size() > ef) top.pop();
                lb = top.top().first;
                ++stats.beam_update_count;
                return true;
            }
        }
        return false;
    ...
    // ------------------------------------------------------------------
    {
        const uint32_t ep = projection_ep_;
        const float    d  = distance_->compare(
            data_bp_ + ep * dimension_, query, (unsigned)dimension_);
        ++stats.distance_cmps;
        ++stats.cur_distance_cmps;
        ++stats.unique_nn_scored;
        ++stats.beam_candidate_count;
    ...
    // ------------------------------------------------------------------
    while (!cand.empty()) {
        //终止条件：最优候选都比已知第ef近的结果差，搜索收敛
        const DistId cur = cand.top();
        ...
        // ---- Step 1: 收集 cur 的所有未入队邻居 mid，计算距离，排序 ----
        struct MidCand { uint32_t id; float dist; };
        std::vector<MidCand> mid_cands;
        mid_cands.reserve(projection_graph_[cur_id].size());

        for (uint32_t mid : projection_graph_[cur_id]) {
            if (state[mid] == ENQUEUED) {
                ++stats.total_already_visited;
                ++stats.duplicate_nn_skip;
                continue;
            }
            ...
            float mid_dist;
            if (state[mid] == UNSEEN) {
                mid_dist = distance_->compare(
                    data_bp_ + mid * dimension_, query, (unsigned)dimension_);
                ++stats.distance_cmps;
                ++stats.mid_distance_cmps;
                ++stats.unique_nn_scored;
                dist_cache[mid] = mid_dist;
                state[mid]      = SCORED;
            } else {
                // SCORED：直接读 cache
                ++stats.duplicate_nn_skip;
                mid_dist = dist_cache[mid];
            }
            mid_cands.push_back({mid, mid_dist});
        }

        // 按距离升序
        std::sort(mid_cands.begin(), mid_cands.end(),
                  [](const MidCand &a, const MidCand &b) {
                      return a.dist < b.dist;
                  });

        // ---- Step 2: best_mid_dist = 本次展开中最近 mid 的距离 ----
        const float best_mid_dist = mid_cands.empty()
            ? std::numeric_limits<float>::infinity()
            : mid_cands[0].dist;

        // ---- 非 jump 模式：直接入队所有 mid ----
        if (!enable_jump) {
            for (const MidCand &mc : mid_cands) {
                ++stats.non_jump_mid_count;

                // non-jump-mid 度数统计
                const uint32_t deg = static_cast<uint32_t>(
                    projection_graph_[mc.id].size());
                stats.non_jump_mid_degree_min     = std::min(stats.non_jump_mid_degree_min, deg);
                stats.non_jump_mid_degree_max     = std::max(stats.non_jump_mid_degree_max, deg);
                stats.non_jump_mid_degree_sum    += deg;
                stats.non_jump_mid_degree_sum_sq +=
                    static_cast<double>(deg) * deg;
                ++stats.non_jump_mid_degree_count;

                enqueue_node(mc.id, mc.dist);
            }
            continue;
        }

        // ---- Step 3: jump 模式：逐 mid 判断是否有 jump 邻居 ----
        for (const MidCand &mc : mid_cands) {
            const uint32_t mid_id   = mc.id;
            const float    mid_dist = mc.dist;
            const uint32_t mid_deg  = static_cast<uint32_t>(
                projection_graph_[mid_id].size());

            struct JumpCand { uint32_t nn; float nn_dist; };
            std::vector<JumpCand> jump_cands;
            jump_cands.reserve(mid_deg);

            uint32_t nn_cmps_this_mid = 0;
            bool     has_jump         = false;

            for (uint32_t nn : projection_graph_[mid_id]) {
                if (state[nn] == ENQUEUED) continue;

                float nn_dist;
                if (state[nn] == UNSEEN) {
                    nn_dist = distance_->compare(
                        data_bp_ + nn * dimension_, query, (unsigned)dimension_);
                    ++stats.distance_cmps;
                    ++nn_cmps_this_mid;
                    ++stats.unique_nn_scored;
                    dist_cache[nn] = nn_dist;
                    state[nn]      = SCORED;
                } else {
                    // SCORED：cache 命中
                    ++stats.duplicate_nn_skip;
                    nn_dist = dist_cache[nn];
                }
                // jump 判断：nn 比当前所有 mid 都近，且比 cur 本身还近
                const bool is_jump = (nn_dist < best_mid_dist && nn_dist < cur_dist);
                ...
                if (is_jump) {
                    has_jump = true;
                    jump_cands.push_back({nn, nn_dist});
                }
            }
            ...
            if (has_jump) {
                ...
                if (mid_dist < cur_dist) ++jump_mid_closer_count;
                else if (mid_dist > cur_dist) ++jump_mid_farther_count;

                // jump-mid 度数统计
                stats.jump_mid_degree_min     = std::min(stats.jump_mid_degree_min, mid_deg);
                stats.jump_mid_degree_max     = std::max(stats.jump_mid_degree_max, mid_deg);
                stats.jump_mid_degree_sum    += mid_deg;
                stats.jump_mid_degree_sum_sq +=
                    static_cast<double>(mid_deg) * mid_deg;
                ++stats.jump_mid_degree_count;

                // 按距离升序，取前 max_jump_cands 个
                std::sort(jump_cands.begin(), jump_cands.end(),
                          [](const JumpCand &a, const JumpCand &b) {
                              return a.nn_dist < b.nn_dist;
                          });
                size_t push_cnt = jump_cands.size();
                if (max_jump_cands > 0)
                    push_cnt = std::min(push_cnt,
                                        static_cast<size_t>(max_jump_cands));

                for (size_t x = 0; x < push_cnt; ++x)
                    enqueue_node(jump_cands[x].nn, jump_cands[x].nn_dist);
                ...
                // non-jump-mid 度数统计
                stats.non_jump_mid_degree_min     = std::min(stats.non_jump_mid_degree_min, mid_deg);
                stats.non_jump_mid_degree_max     = std::max(stats.non_jump_mid_degree_max, mid_deg);
                stats.non_jump_mid_degree_sum    += mid_deg;
                stats.non_jump_mid_degree_sum_sq +=
                    static_cast<double>(mid_deg) * mid_deg;
                ++stats.non_jump_mid_degree_count;

                enqueue_node(mid_id, mid_dist);
            }
        }
    }
