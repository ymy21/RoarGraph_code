...
IndexBipartite::SearchStats IndexBipartite::SearchRoarGraph(
    ...
    while (search_queue.has_unexpanded_node()) {
        ...
        for (size_t j = 0; j < projection_graph_[cur_id].size(); ++j) {
            ...
            // mid 仍保持和原 greedy 一致：发现即 visited
            ...
            if (!jump_candidates.empty()) {
                ...
                // 当前统计：只有 jump 成立时，这个 mid 的所有 nn 距离计算才计入总 distance_cmps
...
IndexBipartite::SearchStats IndexBipartite::SearchRoarGraphNoOpt(
    ...
    while (!cand.empty()) {
        ...
        for (size_t j = 0; j < projection_graph_[cur_id].size(); ++j) {
            ...
            if (!visited[nbr]) {
                ...
                if (top.size() < ef || distance < lb) {
                    cand.emplace(distance, nbr);
                    top.emplace(distance, nbr);
                    if (top.size() > ef) {
                        top.pop();
                    }
                    lb = top.top().first;
    ...
    stats.jump_mid_count = 0;
    stats.non_jump_mid_count = 0;
    return stats;
}
